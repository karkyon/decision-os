...
# ============================================================
section "1. 現在の IssueDetail.tsx の状態確認"
# ============================================================
info "ファイルサイズ: $(wc -l < $ISSUE_DETAIL) 行"

HAS_TRACE_PANEL=$(grep -c "TracePanel\|トレース\|trace" $ISSUE_DETAIL 2>/dev/null || echo "0")
HAS_TRACE_API=$(grep -c "/trace/" $ISSUE_DETAIL 2>/dev/null || echo "0")
HAS_RIGHT_PANEL=$(grep -c "右\|rightPanel\|sidePanel\|width.*300\|width.*320\|width.*360" $ISSUE_DETAIL 2>/dev/null || echo "0")

info "trace 関連コード: $HAS_TRACE_PANEL 箇所"
info "trace API呼び出し: $HAS_TRACE_API 箇所"
info "右パネル layout: $HAS_RIGHT_PANEL 箇所"

# ============================================================
section "2. trace API 疎通確認"
# ============================================================
# トークン取得
...
# 既存ISSUEの最初のIDで試す
ISSUE_RESP=$(curl -s http://localhost:8089/api/v1/issues \
  -H "Authorization: Bearer $TOKEN")
ISSUE_ID=$(echo $ISSUE_RESP | python3 -c "
import sys, json
...
if [ -n "$ISSUE_ID" ]; then
  info "テスト用 issue_id: $ISSUE_ID"
  TRACE_RESP=$(curl -s "http://localhost:8089/api/v1/trace/$ISSUE_ID" \
    -H "Authorization: Bearer $TOKEN")
  info "trace API レスポンス: $(echo $TRACE_RESP | head -c 300)"
  ok "trace API 疎通確認完了"
else
  warn "ISSUEが0件。trace APIテストをスキップ"
fi

# ============================================================
section "3. IssueDetail.tsx — 右パネル（トレーサー）を完全実装"
# ============================================================
info "IssueDetail.tsx をバックアップ..."
cp $ISSUE_DETAIL ${ISSUE_DETAIL}.bak_tracer
...
export default function IssueDetail() {
  ...
  useEffect(() => {
    ...
    client.get(`/trace/${id}`)
      ...
      .catch(() => setTraceError('トレースデータを取得できませんでした'))
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const res = await client.patch(`/issues/${issue.id}`, { status: newStatus })
      setIssue(res.data)
      setEditStatus(res.data.status)
    } catch {
  ...
  const statusInfo = STATUS_MAP[issue.status] ?? { label: issue.status, color: '#94a3b8', bg: '#1e293b' }
  const priorityInfo = PRIORITY_MAP[issue.priority] ?? { label: issue.priority, color: '#94a3b8' }
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <button onClick={() => navigate('/')}
            style={{ background: 'none', border: 'none', color: '#60a5fa', cursor: 'pointer', fontSize: '20px' }}>
            ⚖️
          </button>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <MetaRow label="種別" value={issue.issue_type ?? '—'} />
              <MetaRow label="担当者" value={issue.assignee_id ?? '未割当'} />
              <MetaRow label="期限" value={issue.due_date
                ? new Date(issue.due_date).toLocaleDateString('ja-JP')
                : '未設定'} />
              <MetaRow label="更新日" value={new Date(issue.updated_at).toLocaleDateString('ja-JP')} />
            </div>
          </div>
          ...
          {traceLoading ? (
            ...
            <div style={{
              ...
              <div style={{ marginTop: '8px', color: '#64748b', fontSize: '12px' }}>
                このISSUEはINPUTから直接作成されたか、<br/>トレースデータが未登録です。
              </div>
            </div>
          ) : trace ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              {trace.item && (
                <>
                  ...
                  <TraceNode
                    icon="🧩"
                    label={`ITEM（分解単位） — ${trace.item.intent_code} / ${trace.item.domain_code}`}
                    color="#10b981"
                ...
                <>
                  ...
                  >
                    <div style={{ padding: '12px 14px', borderBottom: '1px solid #334155' }}>
                      <div style={{ fontSize: '10px', color: '#f59e0b', fontWeight: '700', marginBottom: '4px', letterSpacing: '0.05em' }}>
                        📄 INPUT（原文） — {trace.input.source_type}
                      </div>
                    ...
                    <div style={{ padding: '12px 14px' }}>
                      <p style={{
                        margin: 0, fontSize: '13px', color: '#cbd5e1',
              ...
              {/* トレースが部分的な場合 */}
              {!trace.action && !trace.item && !trace.input && (
                <div style={{
                  marginTop: '12px', padding: '12px', borderRadius: '8px',
                  background: '#1e293b', border: '1px solid #334155',
                  color: '#64748b', fontSize: '12px', textAlign: 'center',
                }}>
                  この課題はINPUTから直接作成されました
                </div>
              )}
            ...
            <div style={{
              textAlign: 'center', padding: '40px', color: '#64748b', fontSize: '13px',
            }}>
              トレースデータがありません
            </div>
          )}
...
function TraceArrow({ reason }: { reason: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '2px 14px' }}>
...
function TraceNode({
  icon, label, color, title, body, meta
}: {
  icon: string
  label: string
  color: string
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '13px', color: '#f1f5f9', fontWeight: '500', lineHeight: 1.5 }}>
        {title}
      </div>
      {body && (
...
ok "IssueDetail.tsx — トレーサー右パネル 完全実装完了"
...
if npm run build > /tmp/tracer_build.log 2>&1; then
  ok "フロントエンドビルド成功 🎉"
else
  echo ""
  warn "ビルドエラーあり:"
  grep -E "error TS|Error" /tmp/tracer_build.log | head -20
  echo ""
  echo "--- ログ全体 ---"
  cat /tmp/tracer_build.log
fi
...
echo ""
echo "  🌐 ブラウザで確認:"
echo "     http://localhost:3008/issues/<任意のISSUE_ID>"
echo ""
echo "  確認ポイント:"
echo "     ① 画面が左右2カラムで表示される"
echo "     ② 右パネル「意思決定トレーサー」が表示される"
echo "     ③ ISSUE → ACTION → ITEM → INPUT の連鎖が縦に表示される"
echo "     ④ INPUT 原文が長い場合「全文を表示」ボタンが出る"
echo "     ⑤ ステータスバッジをクリックしてステータス変更できる"
echo ""
echo "  テスト用ISSUE URL (最初の1件):"
# 確認用URLを出力
cd $PROJECT_DIR/backend 2>/dev/null || true
source .venv/bin/activate 2>/dev/null || true
ISSUE_ID_FOR_URL=$(curl -s http://localhost:8089/api/v1/issues \
...
try:
    d = json.load(sys.stdin)
    items = d if isinstance(d, list) else d.get('items', d.get('data', []))
    print(items[0]['id'] if items else 'なし')
except: print('取得失敗')
" 2>/dev/null || echo "取得失敗") 
echo "     http://localhost:3008/issues/$ISSUE_ID_FOR_URL"
echo ""
echo "=============================================="
echo "🎉 課題詳細 トレーサー右パネル 実装完了！"
echo "=============================================="
